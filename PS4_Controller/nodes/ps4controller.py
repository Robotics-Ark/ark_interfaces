...
class PS4Controller(BaseNode):
    ...
    def __init__(self, node_name: str, global_config=None):
        ...
        if self.controller is None:
            log.error("❌ No controller detected! Make sure it is connected via Bluetooth or USB.")
            exit()
    ...
    def pack(self):
        ...
        msg.l3_left_stick_click = 1 if self.button_states['L3 (Left Stick Click)'] else 0
        msg.r3_right_stick_click = 1 if self.button_states['R3 (Right Stick Click)'] else 0
    ...
    def step(self):
        ...
        if self.lock:
            ...
            activations = np.array([self.axis_states['Left Stick - Horizontal'] - 128,
            ...
            if np.max(np.abs(activations)) >= 10:
                log.warn("High initial activations detected, controller is locked. Please move the joysticks and triggers.")
                return
    ...
    def handle_button(self, event):
        ...
        self.button_states[self.button_map.get(event.code, event.code)] = bool(event.value)
